...
public class MMOItemsHook {
    ...
    public static ItemStack getItem(String itemStr) {
        ...
        if (itemStr.startsWith("MMOITEMS:") && enabled) {
            try {
                ...
                if (split.length >= 3) {
                    ...
                    int amount = parseAmount(split, 3);
        ...
        if (mat != null) {
            return new ItemStack(mat, parseAmount(split, 1));
        }

        return null;
    }

    private static int parseAmount(String[] split, int index) {
        if (split.length <= index) {
            return 1;
        }
        try {
            return Math.max(1, Integer.parseInt(split[index]));
